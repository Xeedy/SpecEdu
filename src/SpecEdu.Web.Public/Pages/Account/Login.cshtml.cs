...
{
    private readonly string _appUrl = configuration["AppUrl"] ?? "https://localhost:5001";
    ...
    public async Task<IActionResult> OnPostAsync(string? returnUrl = null)
    {
        ...
        if (result.Succeeded)
        {
            ...
            // If returnUrl is local (default), redirect to app dashboard
            if (returnUrl == "/" || returnUrl == Url.Content("~/"))
            {
                return Redirect($"{_appUrl}/Dashboard");
            }
            // If returnUrl looks like an app URL, redirect there
            return Redirect(returnUrl.StartsWith("http") ? returnUrl : $"{_appUrl}{returnUrl}");
        }
